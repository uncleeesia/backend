...
class ServiceController:
    ...
    def extract_service(self, service_id: int | None = None, user_id: int | None = None) -> list[Service] | Exception:
        """"""
        ...
        try:
            ...
            if service_id:
                ...
                sql_command = sql.SQL("""SELECT DISTINCT username, service_id, service_name, by_user_id, price, duration, service_description, service_tags, s.picture_url, listing_timestamp FROM {}.service  s inner join {}.general_user u ON s.by_user_id = u.user_id WHERE service_id = %s""").format(sql.Identifier(self.schema))
                para = (service_id,)

            elif user_id:

                sql_command = sql.SQL("""SELECT DISTINCT username, service_id, service_name, by_user_id, price, duration, service_description, service_tags, s.picture_url, listing_timestamp FROM {}.service  s inner join {}.general_user u ON s.by_user_id = u.user_id WHERE by_user_id = %s""").format(sql.Identifier(self.schema))
                para = (user_id,)

            else:

                sql_command = sql.SQL("""SELECT DISTINCT username, service_id, service_name, by_user_id, price, duration, service_description, service_tags, s.picture_url, listing_timestamp FROM {}.service s inner join {}.general_user u ON s.by_user_id = u.user_id """).format(sql.Identifier(self.schema))
                para = ()
