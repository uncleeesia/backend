...
class PaymentController():
    ...
    def create_payment(self, payment_details: dict) -> Payment | Exception:
        ...
        try:
            ...
            # Validate input using PaymentCreate (no payment_id)
            pending_payment = Payment.model_validate(payment_details)

            sql_command = sql.SQL("""
                INSERT INTO {}.payment (service_id, from_user_id, to_user_id, price, payment_timestamp, booking_timestamp)
                VALUES (%(service_id)s, %(from_user_id)s, %(to_user_id)s, %(price)s, %(payment_timestamp)s, %(booking_timestamp)s)
            ...
            elif not isinstance(callToDB_result, tuple):
